...
public class LineTranslator
{
		...
		switch(getSystemCallName(word))
		{
			case "loadObject":
				...
				if(getDataType(objectToLoad,preload).equals("movable"))
				{
					...
					newCode+="this."+objectToLoad+".radiusX=100;\n";
					newCode+="this."+objectToLoad+".radiusY=100;\n";
				...
				else if(getDataType(array,preload).equals("movableArray"))
				{
					...
					newCode+=child+".radiusX=100;\n";
					newCode+=child+".radiusY=100;\n";
				...
				else if(getDataType(objectToLoad,preload).equals("movable2"))
				{
					...
					newCode+="this.addChild(this."+objectToLoad+");";
					newCode+="\nthis."+objectToLoad+".radiusX=100;";
					newCode+="\nthis."+objectToLoad+".radiusY=100;";
				}
	...
	public static String translateView(String word,String preload)
	{
		...
		{
			String code="if(Math.abs(this."+object1.trim()+".x-this."+object2.trim()+".x)<this."+object1.trim()+".radiusX &&";
			code+="Math.abs(this."+object1.trim()+".y-this."+object2.trim()+".y)<this."+object1.trim()+".radiusY){\n";
			return code;
		}
		else if(isSet(object1.trim(),preload) && isSet(object2.trim(),preload))
		{
			...
			code+="if(Math.abs(this."+object1.trim()+".getChildAt(i).x-this."+object2.trim()+".getChildAt(j).x)<this."+object1.trim()+".getChildAt(i).radiusX &&";
			code+="Math.abs(this."+object1.trim()+".getChildAt(i).y-this."+object2.trim()+".getChildAt(j).y)<this."+object1.trim()+".getChildAt(i).radiusY){var primary='"+object1.trim()+"';\n var primaryIndex=i;\n var secondary='"+object2.trim()+"'\n var secondaryIndex=j;\nvar index;\n";
			return code;
		}
		else
		{
			...
			code+="if(Math.abs(this."+object1.trim()+".x-this."+object2.trim()+".getChildAt(i).x)<this."+object1.trim()+".radiusX &&";
			code+="Math.abs(this."+object1.trim()+".y-this."+object2.trim()+".getChildAt(i).y)<this."+object1.trim()+".radiusY){\n var primary='"+object2.trim()+"';";	
			return code;
		}
	}
	...
	public static String translateObjectMethodCall(String word,String preload)
	{
		...
		else if(method.indexOf("follow")>=0)
		{
			...
			{
				check="if(primary=='"+object.trim()+"')\n{index=i;}else { index=j;} ";
				object=object+".getChildAt(index)";
			}
			String rotation="Kiwi.Utils.GameMath.angleBetween(this."+object+".x,this."+object+".y,this."+arguments[0]+".x,this."+arguments[0]+".y)";
			String code=check+"this."+object+".physics.velocity.x=Math.cos("+rotation+")*"+arguments[1]+";\n";
			if(getDataType(original.trim(),preload).equals("movable2")||getDataType(original.trim(),preload).equals("movable2Array"))
			 	code+="this."+object+".physics.velocity.y=Math.sin("+rotation+")*"+arguments[1]+";\n";
			System.out.println(")))----))))"+getDataType(original.trim(),preload));
			return code;
		}
		...
		else if(method.indexOf("scaleX")>=0)
		{
			if(object.indexOf("[")>=0)
			{
				String subScript=object.substring(object.indexOf("[")+1,object.indexOf("]"));
				object=object.substring(0,object.indexOf("["));
				return "this."+object+".getChildAt("+subScript+").scaleX="+arguments[0]+";";
			}
			else if(isArray(object.trim(),preload))
			{
				String check="if(primary=='"+object.trim()+"')\n{index=i;}else { index=j;} ";
				return check+"\nthis."+object+".getChildAt(index).scaleX="+arguments[0]+";";
			}
			return "this."+object+".scaleX="+arguments[0]+";";
		}
		else if(method.indexOf("stick")>=0)
		{
			if(arguments[0].indexOf("camera")>=0)
			{
				String newCode= "this.game.cameras.defaultCamera.transform.x=-1*this."+object+".x+this.game.stage.width*0.5-this."+object+".width*0.5;";
				newCode+="this.game.cameras.defaultCamera.transform.y=-1*this."+object+".y+this.game.stage.width*0.5-this."+object+".height*0.5;";
				return newCode;
			}
		...
		else if(method.indexOf("setRadius")>=0)
		{
			if(object.indexOf("[")>=0)
			{
				String subScript=object.substring(object.indexOf("[")+1,object.indexOf("]"));
				object=object.substring(0,object.indexOf("["));
				return "this."+object+".getChildAt("+subScript+").radiusX="+arguments[0]+";this."+object+".getChildAt("+subScript+").radiusY="+arguments[1];
			}
			return "this."+object+".radiusX="+arguments[0]+";\n this."+object+".radiusY="+arguments[1]+";";
		}
		...
		else if(method.indexOf("shootY")>=0)
		{
			...
			code+=child+".physics.immovable=true;";
			code+="this."+object+".addChildAt("+child+",0);\n var blt=this."+object+".getChildAt(0);";
			code+="blt.physics.velocity.y="+arguments[0]+";"+object+"_shot=true;setTimeout("+object+"_delayBullet,100);}";
			return code;
		}
		...
		{
			return "this."+object+"_once.stop(); this."+object+"_forever.stop();";
		}
